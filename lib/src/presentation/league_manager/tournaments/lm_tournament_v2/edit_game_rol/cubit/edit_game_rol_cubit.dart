...
class EditGameRolCubit extends Cubit<EditGameRolState> {
  ...
  Future<void> _validateInitialData(int leagueId) async {
    ...
    final fieldsRequest = await _fieldService.searchFieldByFilters(
        AddressFilter(
            ...
            leagueId: _match.statusRequestField == 'ACCEPTED' ||
                    _match.statusRequestField == 'SEND'
                ? null
                : state.leagueId,
            status: _match.statusRequestField == 'ACCEPTED' ||
                    _match.statusRequestField == 'SEND' ||
                    state.selectedFieldValue == 0
                ? 0
                : 1));
    final refereeRequest = await _refereeService.searchByFiltersReferee(
        AddressFilter(
            state: state.selectedState,
            leagueId: _match.refereeId != null ? null : state.leagueId,
            status: _match.statusRequestField == 'ACCEPTED' ||
                    _match.statusRequestField == 'SEND' ||
                    state.selectedRefereeValue == 0 ||
                    _match.refereeId == null
                ? 0
                : 1));
    if ((_match.refereeId != null &&
  ...
  Future<void> _applyFilter() async {
    ...
    if (_match.requestFieldId == null) {
      _fieldList.clear();
      final fieldRequest = await _fieldService.searchFieldByFilters(filter);
      _fieldList.addAll(fieldRequest);
    }
    if (_match.requestRefereeId == null) {
      _refereeList.clear();
      final refereeRequest =
          await _refereeService.searchByFiltersReferee(filter);
      _refereeList.addAll(refereeRequest);
    }
    _buildMixedList();
    emit(state.copyWith(
